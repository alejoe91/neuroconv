...
def add_electrical_series(
    ...
    if backend == "zarr":
        ...
        if compression is not None or compression_opts is not None:
            warnings.warn(
                f"`compression` and `compression_opts` are deprecated. "
                f"Use `compression_kwargs=dict(compression='{compression}', compression_opts={compression_opts})` "
                "instead."
            )
            compression_kwargs = dict(compression=compression, compression_opts=compression_opts)
        if compression_kwargs is None:
